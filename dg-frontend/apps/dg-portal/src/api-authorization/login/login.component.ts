...
export class LoginComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    switch (action.path) {
      case LoginActions.Login:
        console.log("LoginActions.Login",action.path,this.getReturnUrl());
        
        await this.login(this.getReturnUrl());
        break;
      case LoginActions.LoginCallback:
        console.log("LoginCallback",action.path);
  ...
  private redirectToProfile(): void {
    console.log('redirectToProfile',ApplicationPaths.IdentityManagePath);
  ...
  private getReturnUrl(state?: INavigationState): string {
    ...
    if (fromQuery &&
      // !(fromQuery.startsWith(`https://localhost:5001/`) ||
  ...
  private redirectToApiAuthorizationPath(apiAuthorizationPath: string) {
    ...
    // component.
    // const redirectUrl = `https://localhost:5001/${apiAuthorizationPath}`;
